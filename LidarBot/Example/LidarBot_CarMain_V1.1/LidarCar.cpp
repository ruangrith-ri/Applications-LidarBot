...
void LidarCar::ControlWheel(int8_t X, int8_t Y, byte A)  // X, Y, A: -7 ~ 7
{
  ...
  M5.Lcd.print("X:" + String(X) + "Y:" + String(Y));
...
void LidarCar::MapDisplay(void) {
  ...
  for (int i = 0; i < 45; i++) {
    ...
    // distance[showAngle]为雷达扫描的直线距离，将其通过cos和sin分解为x
    // y方向上的距离
    disX[showAngle] = (80 + (distance[showAngle] / 70) *
                                cos(3.14159 * showAngle / 180 + 0.13)) *
                      2;
    disY[showAngle] = (100 + (distance[showAngle] / 70) *
...
// Serial.print(" distance[showAngle] =
// ");Serial.print(distance[showAngle]);Serial.print(" disX[showAngle] =
// ");Serial.print(disX[showAngle]);Serial.print(" disY[showAngle] =
// ");Serial.println(disY[showAngle]);
#if 1
    ...
    if ((showAngle >= 180) && (showAngle <= 360)) {
      if ((distance[showAngle] == 250) || (distance[showAngle] == 0) ||
          (distance[showAngle] >= 10000)) {
...
void LidarCar::CarMaze(void) {
  ...
  if ((count >= 10)) ControlWheel(motor_out, motor_y_out, 0);
}
...
void LidarCar::CarCortrol(void) {
   ...
   *      pid的基础概念：
   * http://blog.sina.cn/dpool/blog/s/blog_80f7b8e90101ikk8.html?md=gd
   */
...
//！死区检测，检测前面无路可走，掉头返回
int LidarCar::MazaCom(float error_line, float left_line, float right_line,
                      float front_line) {
  // Serial.print(" error_line = ");Serial.print(error_line);Serial.print("
  // left_line = ");Serial.print(left_line);Serial.print(" right_line =
  // ");Serial.print(right_line);Serial.print(" front_line =
  // ");Serial.println(front_line);

  static int stop_count;
  if (((abs(error_line) <= 4.0) && (front_line >= 180.0)) &&
      (((right_line <= 190)) || (left_line >= 120.0))) {
    motor_out = 0;
    motor_y_out = 0;
    stop_count++;
  ...
  if ((stop_count >= 50) && ((abs(error_line) >= 5.0) ||
                             ((front_line >= 160.0) || (front_line <= 1.0)))) {
    motor_out = 1;
    motor_y_out = 0;
    return 1;
  }
...
void LidarCar::ControlMode(void) {
  ...
  Serial.print("contro_mode = ");
  Serial.println(contro_mode);
    ...
    switch (commandStatus) {
      ...
      default:
        ...
        switch ((commandStatus - 13) % 6) {
          case 0:
            signalValue[(startAngle / 2250) * 22 + ((commandStatus - 13) / 6)] =
                r;
            commandStatus++;
            break;
          case 1:
          ...
          case 2:
            temp += r;
            distance[(startAngle / 2250) * 22 + ((commandStatus - 13) / 6)] =
                temp > 2 ? temp : 250;
            commandStatus++;
            break;
          case 3:
